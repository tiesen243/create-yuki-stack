...
export async function addBase(opts: ProjectOptions) {
  ...
  await Promise.all([
    ...
    fs.cp(new URL('tools', templatePath), 'tools', {
      recursive: true,
      force: true,
    }),
  ])
  ...
  await Promise.all([
    fs.cp(new URL('packages/validators', templatePath), 'packages/validators', {
      recursive: true,
      force: true,
    }),
    fs.cp(new URL('packages/ui', templatePath), 'packages/ui', {
      recursive: true,
      force: true,
    }),
  ])
